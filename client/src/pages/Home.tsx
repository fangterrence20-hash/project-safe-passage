...
const pillars = [
  {
    number: "01",
    title: "Independent Vetting Board",
    swahili: "Bodi Huru ya Uhakiki",
    description: "Ethical, financial, and compliance auditing of recruitment and study agencies before they can earn public trust.",
    icon: ClipboardCheck,
    stat: "140+ agencies screened",
    detail: "A standing review function aligned with Tanzanian legal frameworks and TaESA integration. Each decision is documented, time-stamped, and visible to the public.",
  },
  {
    number: "02",
    title: "Digital Trust Seal & Registry",
    swahili: "Muhuri wa Kijiti wa Uaminifu",
    description: "A public verification layer that lets prospective migrants and students validate an agency from a phone before paying.",
    icon: ScanLine,
    stat: "Blockchain IDs issued",
    detail: "Confirmed registered companies receive a verifiable identity record with a tamper-evident reference, status history, and audit trail.",
  },
  {
    number: "03",
    title: "Field Oversight Layer",
    swahili: "Safu ya Usimamizi wa Nyangani",
    description: "Ground monitoring and survivor support at the transit hubs where fraudulent departures can still be intercepted.",
    icon: Radar,
    stat: "8 corridors monitored",
    detail: "Teams are designed for high-risk locations including JNIA, KIA, Namanga, Tunduma, Mutukula, Kasulu, and Ngara.",
  },
  {
    number: "04",
    title: "Survivor Intelligence Hub",
    swahili: "Kituo cha Ujasusi cha Waathirika",
    description: "Encrypted evidence intake that turns lived experience into actionable intelligence for law enforcement and protection services.",
    icon: Fingerprint,
    stat: "195 hotline connected",
    detail: "The hub links survivor reports to Police Gender & Children Desks and the National Anti-Trafficking Hotline while protecting sensitive identities.",
  },
];
      ...
      <header className="fixed inset-x-0 top-0 z-40 border-b border-white/10 bg-[#071522]/90 backdrop-blur-xl">
        <div className="container flex h-[74px] items-center justify-between gap-6">
          ...
          <nav className="hidden items-center gap-7 lg:flex">
            {[{ id: "mission", label: language === "EN" ? "Mission" : "Dhamira" }, { id: "pillars", label: language === "EN" ? "The pillars" : "Nguzo" }, { id: "registry", label: language === "EN" ? "Registry" : "Usajili" }, { id: "corridors", label: language === "EN" ? "Corridors" : "Njia" }].map((item) => <button key={item.id} onClick={() => scrollTo(item.id)} className="text-[11px] font-medium uppercase tracking-[0.15em] text-slate-300 transition hover:text-white">{item.label}</button>)}
          </nav>
          <div className="hidden items-center gap-3 sm:flex">
            ...
            <button onClick={() => scrollTo("registry")} className="inline-flex items-center gap-2 rounded-full bg-emerald-300 px-4 py-2.5 text-[10px] font-bold uppercase tracking-[0.13em] text-[#082016] transition hover:bg-emerald-200 active:scale-[0.97]">{language === "EN" ? "Search registry" : "Tafuta usajili"}<ArrowRight size={14} /></button>
          </div>
        ...
        <section className="relative isolate min-h-[730px] overflow-hidden bg-[#071522] pt-[74px] text-white">
          ...
          <div className="container relative flex min-h-[656px] flex-col justify-center pb-14 pt-20">
            <div className="max-w-[720px]">
              ...
              <p className="mt-7 max-w-[570px] text-base leading-7 text-slate-300 sm:text-lg">Eradicating migration fraud, dismantling illegal recruitment networks, and protecting youth across international corridors.</p>
            ...
            <div className="mt-16 grid max-w-[900px] grid-cols-1 gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 sm:grid-cols-3">
              {[{ value: "140+", label: "Verified agencies" }, { value: "08", label: "Active corridors monitored" }, { value: "50+", label: "Fraud alerts issued" }].map((stat) => <div key={stat.label} className="bg-[#0a1a29]/80 px-5 py-5 backdrop-blur-sm sm:px-6"><p className="font-display text-3xl font-semibold text-white">{stat.value}</p><p className="mt-1 text-[10px] uppercase tracking-[0.14em] text-slate-400">{stat.label}</p></div>)}
            </div>
          </div>
